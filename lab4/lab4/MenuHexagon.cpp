...
// public
menu_hexagon::menu_hexagon() {}

int menu_hexagon::menu_main() {
	std::cout << "Выберите одно из возможных действий:\n";
	std::cout << "\"0\" - Добавить новый шестиугольник;\n";
	std::cout << "\"1\" - Вывод информации про шестиугольники;\n";
...
void menu_hexagon::new_hexagon() {
	Hexagon new_hex;
	new_hex.initFromDialog();
	memory.push_back(new_hex);
}

void menu_hexagon::hexagon_info() {
	if (memory.size() == 0) {
		std::cout << "no info:(";
	}
	else {
		int i = 0;
		for (Hexagon e : memory) {
			std::cout << "Ячейка " << i << ":\n";
			e.draw();
			i++;
		}
...
double menu_hexagon::sum_square() {
	double ans = 0;
	for (Hexagon e : memory) {
		ans += e.square();
	}
...
double menu_hexagon::sum_perimeter() {
	double ans = 0;
	for (Hexagon e : memory) {
		ans += e.perimeter();
	}
...
CVector2D menu_hexagon::center_of_mass_of_system() {
	double x_sum = 0;
	double y_sum = 0;
	double count = 0;
...
unsigned int menu_hexagon::used_memory() {
	Hexagon s_null;
	return (s_null.size() * memory.size());
}

void menu_hexagon::sort() {
	std::sort(memory.begin(), memory.end(),
		[](const Hexagon& a, const Hexagon& b) {
